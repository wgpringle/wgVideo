...
export function useGeneratedScenes(userId, projectId) {
  ...
  useEffect(() => {
    ...
    const generatedQuery = query(generatedRef, orderBy('createdAt'));
    const unsubscribe = onSnapshot(generatedQuery, (snapshot) => {
  ...
  const addGeneratedScene = useCallback(
    async (payload = {}) => {
      ...
      const newDoc = await addDoc(generatedRef, data);
      return newDoc.id;
    },
